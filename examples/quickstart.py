...
async def main():
    ...
    pipeline.register(
        AuditLogger(backend="json", path="audit.jsonl"),
        priority=40,
        optional=True,
    )
    ...
    print(f"VT1 action: {result.action.value}")  # -> continue
    ...
    print(f"VT2 action: {result2.action.value}")  # -> abort (needs human approval)
    ...
    print(f"VT0 action: {result3.action.value}")  # -> continue
