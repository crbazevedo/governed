...
class RateLimiter(CallbackHandler):
    """Enforces bounded concurrency for pipeline actions.

    Uses a sliding window counter to limit the number of concurrent
    actions within a time window. Actions exceeding the limit are aborted.

    Attributes:
        max_concurrent: Maximum actions allowed in the window.
    ...
    def __init__(
        ...
        self._max_concurrent = max_concurrent or RATE_LIMIT_MAX_CONCURRENT
    ...
    async def check(self, context: CallbackContext) -> CallbackResult:
        ...
        if len(self._timestamps[agent_id]) >= self._max_concurrent:
            return CallbackResult.abort(
                handler_name=self.name,
                reason=(
                    f"Rate limit exceeded for {agent_id}: "
                    f"{len(self._timestamps[agent_id])}/{self._max_concurrent} "
