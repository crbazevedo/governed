...
class TestPIIFilter:
    ...
    async def test_nested_dict_redaction(self):
        ...
        result = await f.evaluate(ctx)
        assert result.action == Verdict.MODIFY
        assert "user@example.com" not in result.modified_context.payload["nested"]["contact"]
    ...
    async def test_original_payload_not_mutated(self):
        ...
        ctx = ActionContext(action="test", payload=original)
        await f.evaluate(ctx)
...
class TestRateLimiter:
    ...
    async def test_exceeds_limit_blocks(self):
        ...
        ctx = ActionContext(action="test", agent_id="bot")
        await rl.evaluate(ctx)  # 1
        await rl.evaluate(ctx)  # 2
        result = await rl.evaluate(ctx)  # 3 -> over limit
        assert result.action == Verdict.BLOCK
        assert "Rate limit exceeded" in result.reason

    async def test_different_agents_independent(self):
        rl = RateLimiter(max_per_window=1, window_seconds=60)
        ctx1 = ActionContext(action="test", agent_id="bot1")
        ctx2 = ActionContext(action="test", agent_id="bot2")
        r1 = await rl.evaluate(ctx1)
        r2 = await rl.evaluate(ctx2)
        assert r1.action == Verdict.ALLOW
        assert r2.action == Verdict.ALLOW

    async def test_backward_compat_max_concurrent(self):
...
class TestComplianceChecker:
    ...
    async def test_rejects_oversized_payload(self):
        ...
        ctx = ActionContext(action="test", agent_id="bot", payload=big_payload)
        result = await cc.evaluate(ctx)
        assert result.action == Verdict.BLOCK
    ...
    async def test_vt1_requires_agent_id(self):
        cc = ComplianceChecker()
        ctx = ActionContext(action="test", agent_id="", vt_tier=1)
        result = await cc.evaluate(ctx)
        assert result.action == Verdict.BLOCK
...
class TestUXHandler:
    async def test_vt0_passes_through(self):
        ux = UXHandler()
        ctx = ActionContext(action="test", vt_tier=0)
        result = await ux.evaluate(ctx)
        assert result.action == Verdict.ALLOW
    ...
    async def test_vt4_escalation_intent(self):
        ux = UXHandler()
        ctx = ActionContext(action="dangerous", agent_id="bot", vt_tier=4)
        result = await ux.evaluate(ctx)
        assert result.action == Verdict.MODIFY
        msg = result.modified_context.metadata.get("hitl_message")
